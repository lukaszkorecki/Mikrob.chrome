...
CollectionStore.prototype.get = function(key) {
  var res, current_data = this.getCurrent();

  if(key instanceof Array) {
    res = key.map(function(single_key){
      var ob = {};
      ob[single_key] = current_data[single_key];
      return ob;
    });

  } else {
    res = current_data[key];

  }
  return res;
};

CollectionStore.prototype.empty = function() {
  localStorage[this.collection_name] = undefined;
};
