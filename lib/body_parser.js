var BodyParser = (function() {
  ...
  function statusLocation(body) {
    return body.replace(findLocations,function(loc){
      ...
      var location_url_small = "http://maps.google.com/maps/api/staticmap?center="+encodeURI(extracted_loc)+"&zoom=14&size=120x120&sensor=true";
      var location_url = "http://maps.google.com/maps/api/staticmap?center="+encodeURI(extracted_loc)+"&zoom=14&size=350x350&sensor=true";

      var tag = ["<span class='pic location'>(Lokacja: <input type='image' src='",location_url_small,"' data-action='picture' data-url='",location_url,"' />)</span> "].join('');
      return tag;
    });
  }
